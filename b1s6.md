...
def safeSqrt(x: Double): Try[Double] = {
  if (x < 0) {
    ...
    // Failure(exception): Indica que la operación falló y contiene una excepción que describe el error que ocurrió.
  } else {
    Success(math.sqrt(x))
    // Success(value): Indica que la operación fue exitosa y contiene un valor, en este caso, un valor de tipo Double.
  }
}
